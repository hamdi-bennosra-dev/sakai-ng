...
export class ReservationComponent implements OnInit {
    ...
    groupedReservations: {
        [key: string]: { totalPrice: number; reservations: Reservation[] };
    } = {};
    ...
    private loadReservations(): void {
        ...
        this.reservationService.getAllReservations().subscribe({
            next: (data) => {
                ...
                this.reservations.forEach((r: Reservation) => {
                    const id = r.userId;
                    this.userService.getUserById(id).subscribe({
                        next: (u: User) => {
                            r.userId = u.username + ' - ' + u.email;
                        },
                    });
                });
    ...
    private groupReservations(): void {
        ...
        this.reservations.forEach((reservation) => {
            const loanedOnDate = new Date(
                reservation.loanedOn
            ...
            if (!this.groupedReservations[groupKey]) {
                this.groupedReservations[groupKey] = {
                    totalPrice: 0,
                    reservations: [],
                };
            }
            ...
            this.groupedReservations[groupKey].totalPrice +=
    ...
    saveReservation(): void {
        if (this.selectedReservation) {
            ...
            this.selectedReservation.loanedFrom = new Date(
                this.selectedReservation.loanedFrom
            ).toISOString();
            this.selectedReservation.loanedUntil = new Date(
                this.selectedReservation.loanedUntil
            ).toISOString();

            this.reservationService
                .createOrUpdateReservation(this.selectedReservation)
                .subscribe(() => {
                    this.closeDialog();
                    this.loadReservations();
                });
